...
# --- FINAL, ROBUST Network Diagram Logic ---
def create_network_diagram(df):
    ...
    # --- NEW, SMARTER LAYOUT LOGIC ---
    pos = {}
    # Count tasks at each ES level
    es_counts = defaultdict(int)
    for _, row in df.iterrows():
        es_counts[row['ES']] += 1

    # Position nodes, spreading them out vertically
    y_pos_track = defaultdict(lambda: 0)
    for _, row in df.iterrows():
        es = row['ES']
        count_at_es = es_counts[es]
        
        # Calculate the starting y-position to center the column of nodes
        start_y = -((count_at_es - 1) * 1.5) / 2
        
        # Get the current position in the stack and update it for the next node
        current_y_offset = y_pos_track[es]
        pos[row['Task ID']] = (es, start_y + current_y_offset)
        y_pos_track[es] += 1.5


    # --- ADAPTIVE VISUALS ---
    is_large_graph = len(df) > 25
    node_size = 25 if is_large_graph else 45
    standoff_dist = 15 if is_large_graph else 25
        ...
        if edge[0] in pos and edge[1] in pos:
            ...
            arrow_annotations.append(
                go.layout.Annotation(dict(
                    ...
                    showarrow=True, arrowhead=3, arrowsize=1.5, arrowwidth=1, arrowcolor='#888',
    ...
    for node in G.nodes():
        if node in pos:
            ...
            if not task_info.empty:
                ...
                node_color.append('grey'); node_text.append(f"Task: {node} (Missing)")

    node_trace = go.Scatter(
        x=node_x, y=node_y, mode='markers' if is_large_graph else 'markers+text',
        text=text_inside_node, textposition="middle center", hovertext=node_text, hoverinfo='text',
        textfont=dict(color='white', size=10),
        marker=dict(color=node_color, size=node_size, line=dict(color='Black', width=1))
    )

    layout = go.Layout(
        title=dict(text='<br>CPM Network Diagram', font=dict(size=16)), showlegend=False,
